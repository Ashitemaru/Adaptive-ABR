...
class RNNMPCController(Policy, Serializable):
    def __init__(
        ...
        if not self.use_reward_model:
            assert hasattr(
                self.unwrapped_env, "reward"
            ), "env must have a reward function"
    ...
    def get_actions(self, observations):
        ...
        _, self._hidden_state = self.dynamics_model.predict(
            np.array(observations), actions, self._hidden_state
        )
    ...
    def get_cem_action(self, observations):
        ...
        for i in range(self.num_cem_iters):
            ...
            for t in range(h):
                next_observation, hidden_state = self.dynamics_model.predict(
                    observation, a[t], hidden_state
                )
                if self.use_reward_model:
                    assert self.reward_model is not None
                    rewards = self.reward_model.predict(
                        observation, a[t], next_observation
                    )
                else:
                    rewards = self.unwrapped_env.reward(
                        observation, a[t], next_observation
                    )
    ...
    def reset(self, dones=None):
        ...
        if self._hidden_state is None:
            self._hidden_state = self.dynamics_model.get_initial_hidden(
        ...
        if not isinstance(hidden, list) and not isinstance(hidden, tuple):
            if isinstance(hidden, LSTMStateTuple):
                ...
                return LSTMStateTuple(
                    np.repeat(hidden_c, n, axis=0), np.repeat(hidden_h, n, axis=0)
                )
