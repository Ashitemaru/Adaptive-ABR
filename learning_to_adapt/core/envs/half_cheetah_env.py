...
class HalfCheetahEnv(MujocoEnv, Serializable):
    def __init__(self, task="cripple", reset_every_episode=False):
        ...
        MujocoEnv.__init__(
            self,
            os.path.join(
                os.path.abspath(os.path.dirname(__file__)), "assets", "half_cheetah.xml"
            ),
        )
    ...
    def reset_task(self, value=None):
        if self.task == "cripple":
            crippled_joint = (
                value if value is not None else np.random.randint(1, self.action_dim)
            )
            self.cripple_mask = np.ones(self.action_space.shape)
            self.cripple_mask[crippled_joint] = 0
            geom_idx = self.model.geom_names.index(
                self.model.joint_names[crippled_joint + 3]
            )
