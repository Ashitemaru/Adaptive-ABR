...
class Arm7DofEnv(MujocoEnv, Serializable):
    def __init__(self, task="force", reset_every_episode=False, fixed_goal=False):
        ...
        MujocoEnv.__init__(
            self,
            os.path.join(
                os.path.abspath(os.path.dirname(__file__)), "assets", "arm_7dof.xml"
            ),
        )
    ...
    def reset_mujoco(self, init_state=None):
        if init_state is None:
            low = np.array([-0.1, -0.2, 0.5])
            high = np.array([0.4, 0.2, -0.5])
            ...
            qvel = self.init_qvel + np.random.uniform(
                low=-0.005, high=0.005, size=self.init_qvel.shape
            )
            qvel[-3:, 0] = 0
    ...
    def reset_task(self, value=None):
        if self.task == "cripple":
            ...
            g = value if value is not None else np.random.uniform(0.1, 2)
            masses = self._init_body_masses
