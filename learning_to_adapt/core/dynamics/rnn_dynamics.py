from core.dynamics.core.layers import RNN
...
class RNNDynamicsModel(Serializable):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            self.obs_ph = tf.placeholder(
                tf.float32, shape=(None, None, obs_space_dims), name="obs_ph"
            )
            self.act_ph = tf.placeholder(
                tf.float32, shape=(None, None, action_space_dims), name="act_ph"
            )
            self.delta_ph = tf.placeholder(
                tf.float32, shape=(None, None, obs_space_dims), name="delta_ph"
            )
            ...
            self._gradients_ph = [
                tf.placeholder(shape=param.shape, dtype=tf.float32) for param in params
            ]
    ...
    def _data_input_fn(self, obs, act, delta, batch_size=500, buffer_size=100000):
        ...
        assert (
            obs.shape[0] == act.shape[0] == delta.shape[0]
        ), "inputs must have same length along axis 0"
        assert (
            obs.shape[1] == act.shape[1] == delta.shape[1]
        ), "inputs must have same length along axis 1"
        assert (
            obs.shape[2] == delta.shape[2]
        ), "obs and obs_next must have same length along axis 1 "
    ...
    def get_initial_hidden(self, batch_size):
        ...
        if isinstance(state, tuple) and not isinstance(
