...
def make_plot(
    ...
    for idx, plt in enumerate(plot_list):
        ...
        if use_median:
            ...
            y_upper = list(plt.percentile75)
            y_lower = list(plt.percentile25)
            y_extras = [list(ys) for ys in [plt.percentile0, plt.percentile100]]
            ...
            if len(filtered_data) > 0:
                if only_show_best or only_show_best_final or only_show_best_sofar:
                    ...
                    if best_regret != -np.inf:
                        ...
                        if use_median:
                            ...
                            if clip_plot_value is not None:
                                percentile25 = np.clip(
                                    percentile25, -clip_plot_value, clip_plot_value
                                )
                                percentile50 = np.clip(
                                    percentile50, -clip_plot_value, clip_plot_value
                                )
                                percentile75 = np.clip(
                                    percentile75, -clip_plot_value, clip_plot_value
                                )
                            to_plot.append(
                                AttrDict(
                            ...
                            percentile0 = np.nanpercentile(progresses, q=0, axis=0)
                            percentile25 = np.nanpercentile(progresses, q=25, axis=0)
                            percentile50 = np.nanpercentile(progresses, q=50, axis=0)
                            percentile75 = np.nanpercentile(progresses, q=75, axis=0)
                            percentile100 = np.nanpercentile(progresses, q=100, axis=0)
                            if smooth_curve:
                                percentile0 = sliding_mean(
                                    percentile0, window=window_size
                                )
                                percentile25 = sliding_mean(
                                    percentile25, window=window_size
                                )
                                ...
                                percentile100 = sliding_mean(
                                    percentile100, window=window_size
                                )
                            if clip_plot_value is not None:
                                percentile0 = np.clip(
                                    percentile0, -clip_plot_value, clip_plot_value
                                )
                                percentile25 = np.clip(
                                    percentile25, -clip_plot_value, clip_plot_value
                                )
                                ...
                                percentile100 = np.clip(
                                    percentile100, -clip_plot_value, clip_plot_value
                                )
                            to_plot.append(
                                AttrDict(
                            ...
                            if normalize_error:  # and len(progresses) > 0:
                                stds /= np.sqrt(
                                    np.sum((1.0 - np.isnan(progresses)), axis=0)
                                )
                            if smooth_curve:
                            ...
                            if clip_plot_value is not None:
                                means = np.clip(
                                    means, -clip_plot_value, clip_plot_value
                                )
                    ...
                    if show_lowest_sofar:
                        progresses = [
                            np.array([np.min(ps[: i + 1]) for i in range(len(ps))])
                            for ps in progresses
                        ]
                    if show_highest_sofar:
                        progresses = [
                            np.array([np.max(ps[: i + 1]) for i in range(len(ps))])
                            for ps in progresses
                        ]
                    ...
                    if use_median:
                        ...
                        percentile25 = np.nanpercentile(progresses, q=25, axis=0)
                        percentile50 = np.nanpercentile(progresses, q=50, axis=0)
                        mean = np.nanmean(progresses)
                        percentile75 = np.nanpercentile(progresses, q=75, axis=0)
                        percentile100 = np.nanpercentile(progresses, q=100, axis=0)
                        if smooth_curve:
                            percentile0 = sliding_mean(percentile0, window=window_size)
                            percentile25 = sliding_mean(
                                percentile25, window=window_size
                            )
                            percentile50 = sliding_mean(
        ...
        if len(to_plot) > 0 and not gen_eps:
            ...
            if squeeze_nan:
                for to_plot_i in to_plot:
                    to_plot_i.custom_x = custom_x = np.where(
                        np.logical_not(np.isnan(to_plot_i.means))
                    )[0]
...
def reload_data():
    ...
    plottable_keys = sorted(
        list(set(flatten(list(exp.progress.keys()) for exp in exps_data)) - {None})
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", default=False, action="store_true", help="Open a brower tab automatically"
    )
