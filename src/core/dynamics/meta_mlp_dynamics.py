from core.dynamics.core.layers import MLP
...
class MetaMLPDynamicsModel(Serializable):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            self.adaptation_sym = tf.train.GradientDescentOptimizer(
        ...
        for idx in range(self.meta_batch_size):
            with tf.variable_scope(name + "/pre_model_%d" % idx, reuse=tf.AUTO_REUSE):
                ...
                pre_loss = tf.reduce_mean(
                    tf.square(pre_delta_per_task[idx] - pre_delta_pred)
                )
            ...
            with tf.variable_scope(name + "/post_model_%d" % idx, reuse=tf.AUTO_REUSE):
                ...
                post_loss = tf.reduce_mean(
                    tf.square(post_delta_per_task[idx] - post_delta_pred)
                )
    ...
    def fit(
        ...
        if (
            self.normalization is None or compute_normalization
        ...
        (
            ...
            act_test,
            delta_test,
        ) = train_test_split(obs, act, delta, test_split_ratio=valid_split_ratio)
        ...
        num_steps_per_epoch = max(
            int(
                np.prod(self._dataset_train["obs"].shape[:2])
                / (self.meta_batch_size * self.batch_size * 2)
            ),
            1,
        )
        num_steps_test = max(
            int(
                np.prod(self._dataset_test["obs"].shape[:2])
                / (self.meta_batch_size * self.batch_size * 2)
            ),
            1,
        )
        ...
        for epoch in range(epochs):
            ...
            if valid_loss_rolling_average is None:
                valid_loss_rolling_average = (
                    1.5 * valid_loss
                )  # set initial rolling to a higher value avoid too early stopping
    ...
    def predict(self, obs, act):
        ...
        if self.normalize_input:
            ...
            delta = denormalize(
                delta, self.normalization["delta"][0], self.normalization["delta"][1]
            )
        else:
            delta = np.array(self._predict(obs, act))
    ...
    def _predict(self, obs, act):
        if self._adapted_param_values is not None:
            ...
            delta = sess.run(
                self.post_update_delta[: self._num_adapted_models], feed_dict=feed_dict
            )
    ...
    def _pad_inputs(self, obs, act, obs_next=None):
        if self._num_adapted_models < self.meta_batch_size:
            pad = int(
                obs.shape[0]
                / self._num_adapted_models
                * (self.meta_batch_size - self._num_adapted_models)
            )
            ...
            if obs_next is not None:
                obs_next = np.concatenate(
                    [obs_next, np.zeros((pad,) + obs_next.shape[1:])], axis=0
                )
    ...
    def adapt(self, obs, act, obs_next):
        ...
        obs = np.concatenate(
            [np.concatenate([ob, np.zeros_like(ob)], axis=0) for ob in obs], axis=0
        )
        act = np.concatenate(
            [np.concatenate([a, np.zeros_like(a)], axis=0) for a in act], axis=0
        )
        obs_next = np.concatenate(
            [np.concatenate([ob, np.zeros_like(ob)], axis=0) for ob in obs_next], axis=0
        )
    ...
    def switch_to_pre_adapt(self):
        if self._prev_params is not None:
            [
                ...
                for nn, params in zip(self._networks, self._prev_params)
            ]
            self._prev_params = None
            self._adapted_param_values = None
    ...
    def _get_batch(self, train=True):
        if train:
            ...
            idx_batch = np.random.randint(
                self.batch_size, len_path - self.batch_size, size=self.meta_batch_size
            )

            obs_batch = np.concatenate(
                [
                    self._dataset_train["obs"][
                        ip, ib - self.batch_size : ib + self.batch_size, :
                    ]
    ...
    def _normalize_data(self, obs, act, obs_next=None):
        obs_normalized = normalize(
            obs, self.normalization["obs"][0], self.normalization["obs"][1]
        )
        actions_normalized = normalize(
            act, self.normalization["act"][0], self.normalization["act"][1]
        )

        if obs_next is not None:
            delta = obs_next - obs
            deltas_normalized = normalize(
                delta, self.normalization["delta"][0], self.normalization["delta"][1]
            )
    ...
    def _create_placeholders_for_vars(self, vars):
        ...
        for key, var in vars.items():
            placeholders[key] = tf.placeholder(
                tf.float32, shape=var.shape, name=key + "_ph"
            )
    ...
    def network_params_feed_dict(self):
        return dict(
            list(
                (
                    self.network_phs_meta_batch[i][key],
                    self._adapted_param_values[i][key],
                )
                for key in self._adapted_param_values[0].keys()
                for i in range(self._num_adapted_models)
            )
        )
