...
def create_dnn(
    ...
    with tf.variable_scope(name):
        ...
        for idx, (kernel_size, stride, filter) in enumerate(
            zip(kernel_sizes, strides, num_filters)
        ):
...
def create_rnn(
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        for idx, hidden_size in enumerate(hidden_sizes):
            if cell_type == "lstm":
                cell.append(
                    tf.nn.rnn_cell.LSTMCell(hidden_size, activation=hidden_nonlinearity)
                )
                if create_hidden:
                    c = tf.placeholder(
                        tf.float32, (None, hidden_size), name="cell_state_%d" % idx
                    )
                    h = tf.placeholder(
                        tf.float32, (None, hidden_size), name="hidden_state_%d" % idx
                    )
                    state_var.append(tf.contrib.rnn.LSTMStateTuple(c, h))
            elif cell_type == "gru":
                cell.append(
                    tf.nn.rnn_cell.GRUCell(hidden_size, activation=hidden_nonlinearity)
                )
                if create_hidden:
                    h = tf.placeholder(
                        tf.float32, (None, hidden_size), name="hidden_state_%d" % idx
                ...
                cell.append(
                    tf.nn.rnn_cell.RNNCell(hidden_size, activation=hidden_nonlinearity)
                )
                if create_hidden:
                    h = tf.placeholder(
                        tf.float32, (None, hidden_size), name="hidden_state_%d" % idx
        ...
        outputs, next_state_var = tf.nn.dynamic_rnn(
            cell,
            input_var,
