...
class MjModel(MjModelWrapper):
    def __init__(self, xml_path=None, xml_string=None):
        assert (
            xml_path is not None or xml_string is not None
        ), "Must provide either xml_path or xml_string"
        assert (
            xml_path is None or xml_string is None
        ), "Must provide exactly one of xml_path or xml_string"
    ...
    def body_names(self):
        ...
        return [
            ctypes.string_at(start_addr + int(inc)).decode("utf-8")
